			...
			try {
				...
				return function(begin, end) {
					...
					if (size > 0) {
						...
						if (this.charAt) {
							for (i = 0; i < size; i++) {
								cloned[i] = this$1.charAt(start + i);
							}
						} else {
							for (i = 0; i < size; i++) {
								cloned[i] = this$1[start + i];
							}
						}
					}
					return cloned;
...
var add = function (element, userSettings) {
  var newId = guid();
  setId(element, newId);
  ...
  jQuery.fn.perfectScrollbar = function (settingOrCommand) {
    return this.each(function () {
      if (typeof settingOrCommand === 'object' ||
        ...
        if (!instances_1.get(this)) {
          main$1.initialize(this, settings);
        }
      } else {
...
function main () {
  ...
  .then(function (assets) {
    ...
    v.on("viewer.requestClose", function () {
      route("/");
    });
